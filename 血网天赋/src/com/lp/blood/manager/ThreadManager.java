...
public class ThreadManager {
	...
	public static ThreadPool getThreadPool() {
		if (mThreadPool == null) {
			synchronized (ThreadManager.class) {
				if (mThreadPool == null) {
					int cpuCount = Runtime.getRuntime().availableProcessors();// 获取cpu物理线程数
	...
	public static class ThreadPool {
		...
		// 这时候火车站站长下令, 把剩下的5个窗口也打开, 也就是目前已经有10个窗口同时运行. 后来又来了一批人,10个窗口也处理不过来了,而且售票厅人已经满了, 这时候站长就下令封锁入口,不允许其他人再进来,这就是线程异常处理策略.
		...
		// 取消任务
		public synchronized void cancel(Runnable r) {
			if (executor != null) {
				// 从线程队列中移除对象
...
//				executor = null;
		...
		public synchronized void cancel() {
			if (executor != null) {
//				executor.getQueue().remove();
				executor.getQueue().clear();
//				if(!executor.isShutdown()){
//					executor.shutdown();
//				}
//				executor = null;
//				executor.getQueue().removeAll(list);
			}
		}
